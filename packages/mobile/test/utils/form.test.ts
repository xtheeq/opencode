...
import type { FormField } from "@opencode/client/promise";
import {
  formCustom,
  formDisplayValue,
  formInitialValues,
...
const stringField = (
  overrides: Partial<Extract<FormField, { type: "string" }>> = {},
) =>
  ({
    type: "string",
    key: "name",
...
describe("formTextual / formCustom", () => {
  test("number and option-less string are textual", () => {
    ...
    expect(
      formTextual(stringField({ options: [{ value: "o", label: "O" }] })),
    ).toBe(false);
  });
  ...
  test("custom applies to string-with-options and multiselect", () => {
    expect(
      formCustom(
        stringField({ options: [{ value: "o", label: "O" }], custom: true }),
      ),
    ).toBe(true);
    expect(
      formCustom({ type: "multiselect", key: "m", options: [], custom: true }),
    ).toBe(true);
    expect(
      formCustom(stringField({ options: [{ value: "o", label: "O" }] })),
    ).toBe(false);
  });
});
...
describe("formValidateValue", () => {
  ...
  test("string length constraints", () => {
    expect(formValidateValue(stringField({ minLength: 3 }), "ab")).toMatch(
      /at least 3/,
    );
    expect(formValidateValue(stringField({ maxLength: 2 }), "abc")).toMatch(
      /at most 2/,
    );
  });

  test("format constraints", () => {
    expect(formValidateValue(stringField({ format: "email" }), "nope")).toBe(
      "Expected an email address",
    );
    expect(formValidateValue(stringField({ format: "uri" }), "nope")).toBe(
      "Expected a URL",
    );
    expect(
      formValidateValue(stringField({ format: "date" }), "2026-13-01"),
    ).toMatch(/date/);
    expect(
      formValidateValue(stringField({ format: "email" }), "a@b.co"),
    ).toBeUndefined();
  });
  ...
  test("integer rejects decimals", () => {
    expect(formValidateValue({ type: "integer" as const, key: "n" }, 1.5)).toBe(
      "Expected an integer",
    );
    expect(
      formValidateValue({ type: "integer" as const, key: "n" }, 2),
    ).toBeUndefined();
  });

  test("boolean", () => {
    expect(
      formValidateValue({ type: "boolean" as const, key: "b" }, true),
    ).toBeUndefined();
    expect(
      formValidateValue({ type: "boolean" as const, key: "b" }, "yes"),
    ).toBe("Expected yes or no");
  });
  ...
  test("multiselect min/max and non-custom options", () => {
    ...
    expect(formValidateValue(field, ["b"])).toBe(
      "Select only available options",
    );
    expect(
      formValidateValue({ ...field, custom: true }, ["b"]),
    ).toBeUndefined();
  });
...
describe("formToggleMultiselect / formSetMultiselectCustom", () => {
  ...
  test("replaces a previous custom value", () => {
    ...
    expect(formSetMultiselectCustom(undefined, undefined, "new")).toEqual([
      "new",
    ]);
  });
});
...
describe("formSelected / formDisplayValue", () => {
  ...
  test("formDisplayValue renders labels", () => {
    ...
    expect(formDisplayValue(multiField, ["a", "custom"], "(none)")).toBe(
      "A, custom",
    );
