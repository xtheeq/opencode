...
const sessionLocation: LocationRef = {
...
describe("composerLocation", () => {
  test("uses the selected project default for the new-session composer", () => {
    expect(composerLocation("new", undefined, defaultLocation)).toEqual(
      defaultLocation,
    );
  });
  ...
  test("falls back to the default when a real session lacks a location", () => {
    expect(
      composerLocation("ses_abc", { location: undefined }, defaultLocation),
