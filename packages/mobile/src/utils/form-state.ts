...
export function createFormBodyState(form: FormInfo): FormBodyState {
  ...
  return {
    ...
    selected: formSelected(
      form.fields[0],
      initial.answers[form.fields[0]?.key ?? ""],
    ),
...
export function formUnsupported(form: FormInfo): string | undefined {
  if (!Array.isArray(form.fields) || form.fields.length === 0)
    return "This form has no supported fields.";
  for (const field of form.fields as ReadonlyArray<
    FormField | Record<string, unknown>
  >) {
    if (!field || typeof field !== "object" || typeof field.type !== "string")
      return "This form uses an unknown field.";
    if (!("key" in field) || typeof field.key !== "string")
      return "This form uses an invalid field.";
    if ("when" in field && Array.isArray(field.when) && field.when.length > 0)
      return "Conditional forms are not supported yet.";
    if (
      field.type === "string" &&
      "pattern" in field &&
      field.pattern !== undefined
    )
      return "Pattern-constrained forms are not supported yet.";
    if (
      ![
        "string",
        "number",
      ...
      return `Field type ${field.type} is not supported yet.`;
  }
}
...
export function formMove(
  ...
  return {
    ...state,
    selected: (state.selected + direction + total) % total,
    error: "",
  };
}

export function formSetSelected(
  state: FormBodyState,
  selected: number,
): FormBodyState {
...
export function formSetSubmitting(
  state: FormBodyState,
  submitting: boolean,
  error = "",
): FormBodyState {
...
export function formSetExternalReady(
  state: FormBodyState,
  key: string,
): FormBodyState {
  return {
    ...state,
    externalReady: { ...state.externalReady, [key]: true },
    error: "",
  };
}
...
export function formInput(state: FormBodyState, field: FormField | undefined) {
  ...
  return (
    state.custom[field.key] ?? formDisplay(field, state.answers[field.key])
  );
}

export function formSetDraft(
  state: FormBodyState,
  ...
  for (const field of form.fields) {
    if (field.type === "external") {
      if (state.answers[field.key] !== true)
        return `Acknowledge ${formLabel(field)}`;
      continue;
    }
...
export function formReply(
  form: FormInfo,
  state: FormBodyState,
): FormReplyValue | undefined {
  const answer = formAnswer(form, state);
  if (!answer) return;
  return { sessionID: form.sessionID, formID: form.id, answer };
}
...
export function formPick(state: FormBodyState, form: FormInfo): FormBodyState {
  ...
  if (state.selected === rows.length && formCustom(field))
    return formSetEditing(state, true);
  ...
  if (field.type === "multiselect") {
    return {
      ...
      answers: {
        ...state.answers,
        [field.key]: formToggleMultiselect(
  ...
  return formSetField(
    next,
    form,
    formSingle(form) ? state.field : state.field + 1,
  );
}
...
export function formCommitInput(
  ...
  const field = formCurrent(form, state);
  if (!field || field.type === "external" || field.type === "boolean")
    return state;
  ...
  const value = !input
    ...
    : field.type === "number" || field.type === "integer"
      ...
      : input;
  if (field.type === "multiselect") {
    const values = formSetMultiselectCustom(
      state.answers[field.key],
  ...
  return {
    ...
    custom: {
      ...state.custom,
      [field.key]: typeof value === "string" ? value : input,
    },
    editing: false,
    error: "",
  };
...
export function formAcknowledge(
  state: FormBodyState,
  form: FormInfo,
): FormBodyState {
  const field = formCurrent(form, state);
  if (field?.type !== "external" || !state.externalReady[field.key])
    return state;
  const next = {
    ...state,
