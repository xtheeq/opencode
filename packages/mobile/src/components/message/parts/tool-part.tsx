...
function ToolContent({ part }: { part: SessionMessageAssistantTool }) {
  ...
  if (state.content.length === 0 && Object.keys(state.structured).length === 0) return null
  return (
    <>
      {state.content.map((item, i) => {
        switch (item.type) {
          case "text": return <Text key={i} variant="caption">{item.text}</Text>
          case "file": return <Text key={i} variant="caption">{item.name ?? item.mime}: {item.uri}</Text>
        }
